...
export default function Settings() {
    ...
    try {
      ...
      if (settings.id) {
        await updateSettings({
          variables: { id: settings.id, ...payload },
        });
      } else {
        const res = await addSettings({ variables: payload });
        setSettings((prev) => ({
          ...prev,
          id: res.data.insert_admin_settings_one.id,
        }));
      }
  ...
  return (
    <AdminLayout>
      ...
      {/* Error */}
      {error && (
        <p className="text-red-500">Failed to fetch settings</p>
      )}

      {!loading && !error && (
        <form className="max-w-xl space-y-4" onSubmit={handleSave}>
          {[
            ["App Name", "app_name", "text"],
            ["Support Email", "support_email", "email"],
            ["Support Phone", "support_phone", "text"],
            ["Default Currency", "default_currency", "text"],
            ["Tax Percentage", "tax_percentage", "number"],
            ["Commission Percentage", "commission_percentage", "number"],
          ].map(([label, key, type]) => (
            <div key={key}>
